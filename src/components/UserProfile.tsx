...
const UserProfile: React.FC<UserProfileProps> = ({
  ...
  useEffect(() => {
    if (restricted) {
      const profileId = profileIndex !== undefined ? `profile-${profileIndex}` : 'unknown-restricted-profile';
      logAccessAttempt(profileId, 'access_denied');
    }
  }, [restricted, profileIndex]);
  ...
  if (restricted) {
    return (
      <article className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 max-w-sm mx-auto">
